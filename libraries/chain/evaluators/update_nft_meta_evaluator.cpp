...
namespace scorum {
namespace chain {
update_nft_meta_evaluator::update_nft_meta_evaluator(data_service_factory_i& services,
                                                     dba::db_accessor<account_object>& account_dba,
                                                     dba::db_accessor<nft_object>& nft_dba)
    : evaluator_impl<data_service_factory_i, update_nft_meta_evaluator>(services)
...
void update_nft_meta_evaluator::do_apply(const operation_type& op)
{
    ...
    _nft_dba.update(nft, [&](auto& nft) { fc::from_string(nft.json_metadata, op.json_metadata); });
#endif
}
} // namespace chain
} // namespace scorum
