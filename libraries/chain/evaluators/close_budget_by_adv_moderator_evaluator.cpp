#include <scorum/chain/evaluators/close_budget_by_adv_moderator_evaluator.hpp>
...
close_budget_by_adv_moderator_evaluator::close_budget_by_adv_moderator_evaluator(data_service_factory_i& services)
    : evaluator_impl<data_service_factory_i, close_budget_by_adv_moderator_evaluator>(services)
    ...
    , _adv_moderator_service(services.adv_moderator_service())
{
}

void close_budget_by_adv_moderator_evaluator::do_apply(
    const close_budget_by_adv_moderator_evaluator::operation_type& op)
{
    _account_service.check_account_existence(op.moderator);

    FC_ASSERT(_adv_moderator_service.is_exists(), "User ${1} is not the advertising moderator", ("1", op.moderator));
    FC_ASSERT(_adv_moderator_service.get().account_name == op.moderator, "User ${1} is not the advertising moderator",
              ("1", op.moderator));
