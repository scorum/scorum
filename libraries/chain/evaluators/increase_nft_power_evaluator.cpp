...
namespace scorum {
namespace chain {
increase_nft_power_evaluator::increase_nft_power_evaluator(data_service_factory_i& services,
                                                           dba::db_accessor<account_object>& account_dba,
                                                           dba::db_accessor<nft_object>& nft_dba)
    : evaluator_impl<data_service_factory_i, increase_nft_power_evaluator>(services)
...
void increase_nft_power_evaluator::do_apply(const operation_type& op)
{
    FC_ASSERT(_hardfork_service.has_hardfork(SCORUM_HARDFORK_0_5), "Hardfork #5 is required");
    FC_ASSERT(_nft_dba.is_exists_by<by_uuid>(op.uuid), R"(NFT with uuid "${nft_id}" must exist.)", ("nft_id", op.uuid));
    ...
    _nft_dba.update(nft, [&](auto& nft) { nft.power += op.power; });
}
} // namespace chain
} // namespace scorum
