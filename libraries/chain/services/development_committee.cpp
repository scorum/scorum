...
void dbs_development_committee::change_adv_moderator_quorum(const percent_type quorum)
{
    db_impl().modify(get(), [&](dev_committee_object& m) { m.adv_moderator_quorum = quorum; });
}
...
percent_type dbs_development_committee::get_adv_moderator_quorum()
{
    return get().adv_moderator_quorum;
}
