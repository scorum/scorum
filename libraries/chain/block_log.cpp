...
namespace scorum { namespace chain {
   ...
   uint64_t block_log::append( const signed_block& b )
   {
      try
      {
         ...
         FC_ASSERT(
                     (uint64_t)my->index_stream.tellp() == (std::fstream::streampos)sizeof(uint64_t) * ((uint64_t)b.block_num() - 1),
                     "Append to index file occuring at wrong position.",
                     ("position", (uint64_t)my->index_stream.tellp())("expected", ((uint64_t)b.block_num() - 1) * sizeof(uint64_t))
                  );
