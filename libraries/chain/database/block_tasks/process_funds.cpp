...
void process_funds::on_apply(block_task_context& ctx)
{
    ...
    asset dev_team_reward = protocol::multiply_asset_by_fractional(
        advertising_budgets_reward, SCORUM_DEV_TEAM_PER_BLOCK_REWARD_PERCENT, SCORUM_100_PERCENT);
...
void process_funds::distribute_reward(block_task_context& ctx, const asset& users_reward)
{
    ...
    asset witness_reward = protocol::multiply_asset_by_fractional(users_reward , SCORUM_WITNESS_PER_BLOCK_REWARD_PERCENT, SCORUM_100_PERCENT);
    asset active_sp_holder_reward = protocol::multiply_asset_by_fractional(users_reward , SCORUM_ACTIVE_SP_HOLDERS_PER_BLOCK_REWARD_PERCENT ,SCORUM_100_PERCENT);
    ...
    if (!active_sp_holders_array.empty())
    {
        ...
        if (total_sp.amount > 0)
        {
            for (const account_object& account : active_sp_holders_array)
            {
                ...
                asset account_reward = protocol::multiply_asset_by_fractional(
                    total_reward, account.vote_reward_competitive_sp.amount.value, total_sp.amount.value);
