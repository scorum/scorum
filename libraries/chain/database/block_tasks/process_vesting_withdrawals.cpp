...
public:
    ...
    void take_sp_from(const withdrawable_id_type& from, const asset& amount)
    {
        FC_ASSERT(amount.symbol() == VESTS_SYMBOL);

        if (withdraw_vesting_objects::is_account(from))
            take_sp_from(from.get<account_id_type>(), amount);
        else if (withdraw_vesting_objects::is_dev_committee(from))
            take_sp_from(from.get<dev_committee_id_type>(), amount);
        else
        {
            FC_ASSERT(false, "Unknown type");
        }
...
private:
    ...
    void give_sp_to(const dev_committee_id_type& from, const dev_committee_id_type& to, const asset& amount)
    {
        ...
        _dev_pool_service.update([&](dev_committee_object& pool) { pool.sp_balance += amount; });
    }
    ...
    void take_sp_from(const account_id_type& from, const asset& amount)
    {
        FC_ASSERT(amount.amount > 0);

        const account_object& from_account = _account_service.get(from);

        _account_service.update(from_account, [&](account_object& a) { a.vesting_shares -= amount; });

        _account_service.adjust_proxied_witness_votes(from_account, -amount.amount);
    }

    void take_sp_from(const dev_committee_id_type& from, const asset& amount)
    {
        const dev_committee_object& from_pool = _dev_pool_service.get();
...
void process_vesting_withdrawals::on_apply(block_task_context& ctx)
{
    ...
    for (const withdraw_vesting_object& wvo : withdraw_vesting_service.get_until(dprops_service.head_block_time()))
    {
        ...
        actors_impl.take_sp_from(wvo.from_id, to_withdraw);
        ...
        actors_impl.give_scr_to(wvo.from_id, wvo.from_id, converted_scorum);
