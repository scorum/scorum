...
const savings_withdraw_object* database::find_savings_withdraw(
    const account_name_type& owner, uint32_t request_id) const
{
        ...
        if (comment.net_rshares > 0)
        {
            ...
            if (reward_tokens > 0)
            {
                ...
                adjust_total_payout(comment,
                    sbd_payout.first + to_sbd(sbd_payout.second + asset(vesting_scorum, SCORUM_SYMBOL)),
                    to_sbd(asset(curation_tokens, SCORUM_SYMBOL)), to_sbd(asset(total_beneficiary, SCORUM_SYMBOL)));

                push_virtual_operation(author_reward_operation(
                    comment.author, to_string(comment.permlink), sbd_payout.first, sbd_payout.second, vest_created));
                push_virtual_operation(comment_reward_operation(
                    comment.author, to_string(comment.permlink), to_sbd(asset(claimed_reward, SCORUM_SYMBOL))));
...
void database::process_savings_withdraws()
{
    ...
    while (itr != idx.end())
    {
        ...
        modify(get_account(itr->from), [&](account_object& a) { a.savings_withdraw_requests--; });

        push_virtual_operation(fill_transfer_from_savings_operation(
            itr->from, itr->to, itr->amount, itr->request_id, to_string(itr->memo)));
...
asset database::get_liquidity_reward() const
{
    ...
    asset percent(protocol::calc_percent_reward_per_hour<SCORUM_LIQUIDITY_APR_PERCENT>(props.virtual_supply.amount),
        SCORUM_SYMBOL);
...
void database::update_median_feed()
{
    try
    {
        ...
        for (int i = 0; i < wso.num_scheduled_witnesses; i++)
        {
            ...
            if (now < wit.last_sbd_exchange_update + SCORUM_MAX_FEED_AGE_SECONDS && !wit.sbd_exchange_rate.is_null())
            {
        ...
        if (BOOST_LIKELY(head_block_num() > 0))
        {
            if (!(skip & skip_tapos_check))
            {
                ...
                SCORUM_ASSERT(trx.ref_block_prefix == tapos_block_summary.block_id._hash[1],
                    transaction_tapos_exception, "", ("trx.ref_block_prefix", trx.ref_block_prefix)(
...
void database::update_virtual_supply()
{
    try
    {
        modify(get_dynamic_global_properties(), [&](dynamic_global_property_object& dgp) {
            dgp.virtual_supply
                = dgp.current_supply + (get_feed_history().current_median_history.is_null()
            ...
            if (!median_price.is_null())
            {
                auto percent_sbd = uint16_t(
                    ((fc::uint128_t((dgp.current_sbd_supply * get_feed_history().current_median_history).amount.value)
                         * SCORUM_100_PERCENT)
                ...
                if (percent_sbd <= SCORUM_SBD_START_PERCENT)
                    ...
                    dgp.sbd_print_rate = ((SCORUM_SBD_STOP_PERCENT - percent_sbd) * SCORUM_100_PERCENT)
                        / (SCORUM_SBD_STOP_PERCENT - SCORUM_SBD_START_PERCENT);
            }
        });
    }
    FC_CAPTURE_AND_RETHROW()
...
int database::match(const limit_order_object& new_order, const limit_order_object& old_order, const price& match_price)
{
    ...
    else
    {
        ...
        // Although new_order_for_sale is greater than old_order_for_sale * match_price, old_order_for_sale ==
        // new_order_for_sale * match_price
    ...
    push_virtual_operation(fill_order_operation(
        new_order.seller, new_order.orderid, new_order_pays, old_order.seller, old_order.orderid, old_order_pays));
...
bool database::fill_order(const limit_order_object& order, const asset& pays, const asset& receives)
{
    try
    {
        ...
        {
            modify(order, [&](limit_order_object& b) { b.for_sale -= pays.amount; });
            /**
            ...
            if (a.sbd_seconds_last_update != head_block_time())
            {
                ...
                if (acnt.sbd_seconds > 0
                    && (acnt.sbd_seconds_last_update - acnt.sbd_last_interest_payment).to_seconds()
                        > SCORUM_SBD_INTEREST_COMPOUND_INTERVAL_SEC)
                {
...
void database::retally_liquidity_weight()
{
    ...
    {
        modify(i, [](liquidity_reward_balance_object& o) { o.update_weight(); });
    }
}
