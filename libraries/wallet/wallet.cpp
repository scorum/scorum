    ...
    std::map<string, std::function<string(fc::variant, const fc::variants&)>> get_result_formatters() const
    {
        ...
        m["list_my_accounts"] = [](variant result, const fc::variants& a) {
            ...
            for (const auto& a : accounts)
            {
                ...
                out << std::left << std::setw(17) << std::string(a.name) << std::right << std::setw(18)
                    ...
                    << fc::variant(a.vesting_shares).as_string() << " " << std::right << std::setw(16)
            ...
            out << std::left << std::setw(17) << "TOTAL" << std::right << std::setw(18)
                ...
                << fc::variant(total_vest).as_string() << " " << std::right << std::setw(16)
        ...
        m["get_order_book"] = [](variant result, const fc::variants& a) {
            ...
            ss << ' ' << setw((spacing * 4) + 6) << "Bids"
               << "Asks\n"
               << ' ' << setw(spacing + 3) << "Sum(SBD)" << setw(spacing + 1) << "SBD" << setw(spacing + 1) << "SCORUM"
               << setw(spacing + 1) << "Price" << setw(spacing + 1) << "Price" << setw(spacing + 1) << "SCORUM "
               << setw(spacing + 1) << "SBD "
               << "Sum(SBD)"
               << "\n=================================================================================================="
                  "==="
            ...
            for (size_t i = 0; i < orders.bids.size() || i < orders.asks.size(); i++)
            {
                if (i < orders.bids.size())
                {
                    ...
                    ss << ' ' << setw(spacing) << bid_sum.to_string() << ' ' << setw(spacing)
                       << asset(orders.bids[i].sbd, SBD_SYMBOL).to_string() << ' ' << setw(spacing)
                       << asset(orders.bids[i].scorum, SCORUM_SYMBOL).to_string() << ' ' << setw(spacing)
                ...
                if (i < orders.asks.size())
                {
                    ...
                    ss << ' ' << setw(spacing) << orders.asks[i].real_price << ' ' << setw(spacing)
                       << asset(orders.asks[i].scorum, SCORUM_SYMBOL).to_string() << ' ' << setw(spacing)
                       << asset(orders.asks[i].sbd, SBD_SYMBOL).to_string() << ' ' << setw(spacing)
                       << ask_sum.to_string();
                }
            ...
            ss << endl << "Bid Total: " << bid_sum.to_string() << endl << "Ask Total: " << ask_sum.to_string() << endl;
...
vector<account_name_type> wallet_api::get_miner_queue() const { return my->_remote_db->get_miner_queue(); }
...
annotated_signed_transaction wallet_api::escrow_transfer(string from, string to, string agent, uint32_t escrow_id,
    asset sbd_amount, asset scorum_amount, asset fee, time_point_sec ratification_deadline,
...
annotated_signed_transaction wallet_api::escrow_release(string from, string to, string agent, string who,
    string receiver, uint32_t escrow_id, asset sbd_amount, asset scorum_amount, bool broadcast)
{
    FC_ASSERT(!is_locked());
...
annotated_signed_transaction wallet_api::transfer_to_savings(
    string from, string to, asset amount, string memo, bool broadcast)
{
    FC_ASSERT(!is_locked());
    check_memo(memo, get_account(from));
...
/**
 * @param request_id - an unique ID assigned by from account, the id is used to cancel the operation and can be reused
 * after the transfer completes
 */
annotated_signed_transaction wallet_api::transfer_from_savings(
    string from, uint32_t request_id, string to, asset amount, string memo, bool broadcast)
{
    FC_ASSERT(!is_locked());
    check_memo(memo, get_account(from));
...
annotated_signed_transaction wallet_api::claim_reward_balance(
    string account, asset reward_scorum, asset reward_sbd, asset reward_vests, bool broadcast)
{
    FC_ASSERT(!is_locked());
...
annotated_signed_transaction wallet_api::create_order(string owner, uint32_t order_id, asset amount_to_sell,
    asset min_to_receive, bool fill_or_kill, uint32_t expiration_sec, bool broadcast)
{
    FC_ASSERT(!is_locked());
