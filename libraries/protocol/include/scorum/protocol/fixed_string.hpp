...
namespace fc { namespace raw {
   ...
   template< typename Stream, typename Storage >
   inline void unpack( Stream& s, scorum::protocol::fixed_string< Storage >& u )
   {
      std::string str;
      unpack( s, str );
      u = str;
   ...
   void to_variant(   const scorum::protocol::fixed_string< Storage >& s, variant& v ) { v = std::string( s ); }

   template< typename Storage >
   void from_variant( const variant& v, scorum::protocol::fixed_string< Storage >& s ) { s = v.as_string(); }
} // fc
