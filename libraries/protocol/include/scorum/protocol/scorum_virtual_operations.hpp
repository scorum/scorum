...
struct author_reward_operation : public virtual_operation
{
    ...
    author_reward_operation(
        const account_name_type& a, const string& p, const asset& s, const asset& st, const asset& v)
        : author(a)
        , permlink(p)
        , sbd_payout(s)
        , scorum_payout(st)
...
struct fill_order_operation : public virtual_operation
{
    ...
    fill_order_operation(
        const string& c_o, uint32_t c_id, const asset& c_p, const string& o_o, uint32_t o_id, const asset& o_p)
        : current_owner(c_o)
...
struct fill_transfer_from_savings_operation : public virtual_operation
{
    ...
    fill_transfer_from_savings_operation(
        const account_name_type& f, const account_name_type& t, const asset& a, const uint32_t r, const string& m)
        : from(f)
        , to(t)
        , amount(a)
        , request_id(r)
...
FC_REFLECT(scorum::protocol::fill_order_operation,
