...
{ try {
   ...
   if( required_posting.size() ) {
      ...
      {
         SCORUM_ASSERT( s.check_authority(id) ||
   ...
   {
      SCORUM_ASSERT( s.check_authority(auth), tx_missing_other_auth, "Missing Authority", ("auth",auth)("sigs",sigs) );
   }
   ...
   {
      SCORUM_ASSERT( owner_approvals.find(id) != owner_approvals.end() ||
                       s.check_authority(get_owner(id)),
