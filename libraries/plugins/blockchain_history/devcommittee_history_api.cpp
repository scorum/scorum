...
public:
    ...
    void get_history(uint64_t from, uint32_t limit, fill_result_functor& funct) const
    {
        ...
        //        FC_ASSERT(limit <= MAX_BLOCKCHAIN_HISTORY_DEPTH, "Limit of ${l} is greater than maxmimum allowed
        //        ${2}",
        //                  ("l", limit)("2", MAX_BLOCKCHAIN_HISTORY_DEPTH));
        //        FC_ASSERT(from >= limit, "From must be greater than limit");
        //
        //        const auto& idx =
        //        db->get_index<account_history_index<history_object_type>>().indices().get<by_account>();
...
std::map<uint32_t, applied_operation> devcommittee_history_api::get_scr_to_scr_transfers(uint64_t from,
                                                                                         uint32_t limit) const
{
    const auto db = _impl->_app.chain_database();
    return db->with_read_lock(
        [&]() { return _impl->get_history<account_transfers_to_scr_history_object>(from, limit); });
}

std::map<uint32_t, applied_operation> devcommittee_history_api::get_history(uint64_t from, uint32_t limit) const
{
    const auto db = _impl->_app.chain_database();
    return db->with_read_lock([&]() { return _impl->get_history<account_history_object>(from, limit); });
}

std::map<uint32_t, applied_withdraw_operation> devcommittee_history_api::get_sp_to_scr_transfers(uint64_t from,
                                                                                                 uint32_t limit) const
{
    return {};
    //    const auto db = _impl->_app.chain_database();
    ...
    //                        // if last 'progress' operation is 'withdraw_scorumpower_' then withdraw was either
    //                        interrupted
