...
order_book market_history_api_impl::get_order_book(uint32_t limit) const
{
    ...
    const auto& order_idx
        = app.chain_database()->get_index<scorum::chain::limit_order_index>().indices().get<scorum::chain::by_price>();
...
std::vector<market_trade> market_history_api_impl::get_trade_history(
...
std::vector<bucket_object> market_history_api_impl::get_market_history(
...
{
    return my->app.chain_database()->with_read_lock([&]() { return my->get_ticker(); });
}

market_volume market_history_api::get_volume() const
{
    return my->app.chain_database()->with_read_lock([&]() { return my->get_volume(); });
}

order_book market_history_api::get_order_book(uint32_t limit) const
{
    return my->app.chain_database()->with_read_lock([&]() { return my->get_order_book(limit); });
}

std::vector<market_trade> market_history_api::get_trade_history(
    time_point_sec start, time_point_sec end, uint32_t limit) const
{
    return my->app.chain_database()->with_read_lock([&]() { return my->get_trade_history(start, end, limit); });
}

std::vector<market_trade> market_history_api::get_recent_trades(uint32_t limit) const
{
    return my->app.chain_database()->with_read_lock([&]() { return my->get_recent_trades(limit); });
}

std::vector<bucket_object> market_history_api::get_market_history(
    uint32_t bucket_seconds, time_point_sec start, time_point_sec end) const
{
...
{
    return my->app.chain_database()->with_read_lock([&]() { return my->get_market_history_buckets(); });
}
}
