...
SCORUM_TEST_CASE(total_scr_equal_scr_on_accounts_balance)
{
    ...
    account_service.foreach_account([&](const account_object& a) { return expected_total_scr += a.balance; });
...
SCORUM_TEST_CASE(total_sp_equal_sp_on_accounts_balance)
{
    ...
    account_service.foreach_account([&](const account_object& a) { return expected_total_sp += a.scorumpower; });
