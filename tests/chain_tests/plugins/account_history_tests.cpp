...
using operation_map_type = std::map<uint32_t, app::applied_operation>;
...
struct history_database_fixture : public database_fixture::database_trx_integration_fixture
{
    ...
    history_database_fixture()
    {
        ...
        _plugin = app.register_plugin<scorum::account_history::account_history_plugin>();
    ...
    {
        const auto& idx = db.get_index<account_history::history_index<history_object_type>>()
                              .indices()
                              .get<account_history::by_account>();
...
SCORUM_TEST_CASE(check_account_nontransfer_operation_only_in_full_history_test)
{
    ...
    operation_map_type buratino_scr_ops
        = get_operations_accomplished_by_account<account_history::transfers_to_scr_history_object>(buratino);
    operation_map_type buratino_sp_ops
        = get_operations_accomplished_by_account<account_history::transfers_to_sp_history_object>(buratino);
...
SCORUM_TEST_CASE(check_account_transfer_operation_history_test)
{
...

    {
        ...
        operation_map_type maugli_ops
            = get_operations_accomplished_by_account<account_history::transfers_to_scr_history_object>(maugli);
...
SCORUM_TEST_CASE(check_account_transfer_to_scorumpower_operation_history_test)
{
...

    {
        operation_map_type buratino_ops
            = get_operations_accomplished_by_account<account_history::transfers_to_sp_history_object>(buratino);
        operation_map_type maugli_ops
            = get_operations_accomplished_by_account<account_history::transfers_to_sp_history_object>(maugli);
