...
SCORUM_TEST_CASE(more_than_matched_check) // to catch and log exceptions in calculate_matched_stake
{
    ...
    BOOST_REQUIRE_EQUAL(matched.matched_result.amount.value, 222'222'222);
}
...
SCORUM_TEST_CASE(less_than_matched_check)
{
    ...
    BOOST_REQUIRE_EQUAL(matched.matched_result.amount.value, 900'000'000);
}
...
SCORUM_TEST_CASE(equal_matched_check)
{
    ...
    BOOST_REQUIRE_EQUAL(matched.matched_result, stake_my);

    matched = calculate_matched_stake(stake_other, stake_my, odds_other, odds_my);

    BOOST_REQUIRE_EQUAL(matched.matched_result, stake_other);
}
...
SCORUM_TEST_CASE(calculate_matched_stake_vanished_large_check)
{
    ...
    BOOST_REQUIRE_EQUAL(matched.matched_result, ASSET_SCR(1));
}
