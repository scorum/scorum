...
BOOST_FIXTURE_TEST_CASE(create_wincase_pairs_check, market_type_fixture)
{
    ...
    market_type market1 = correct_score_market{};
    ...
    BOOST_CHECK_EQUAL(create_wincase_pairs(market1).size(), 3u);

    BOOST_CHECK_EQUAL(create_wincase_pairs(market2).size(), 1u);

    BOOST_CHECK_EQUAL(create_wincase_pairs(market3).size(), 1u);
}
