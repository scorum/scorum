...
bob_comment_vote_total ), SCORUM_SYMBOL );
...
) / alice_comment.total_vote_weight ), SCORUM_SYMBOL );
...
bob_comment.total_vote_weight ), SCORUM_SYMBOL );
...
dave_comment.total_vote_weight ), SCORUM_SYMBOL );
...
BOOST_AUTO_TEST_CASE(feed_publish_mean)
{
    try
    {
        ...
        BOOST_REQUIRE(
            feed_history.current_median_history == price(asset(99000, SCORUM_SYMBOL), asset(1000, SBD_SYMBOL)));
        ...
        for (int i = 0; i < 23; i++)
        {
            ...
            for (int j = 0; j < 7; j++)
            {
                ...
                ops[j].exchange_rate
                    = price(ops[j].exchange_rate.base, asset(ops[j].exchange_rate.quote.amount + 10, SBD_SYMBOL));
...
BOOST_AUTO_TEST_CASE(scorum_inflation)
{
    try
    {
        /*
           ...
           auto new_rewards = std::max( SCORUM_MIN_CONTENT_REWARD, asset( ( SCORUM_CONTENT_APR *
           gpo.virtual_supply.amount ) / ( SCORUM_BLOCKS_PER_YEAR * 100 ), SCORUM_SYMBOL ) )
              + std::max( SCORUM_MIN_CURATE_REWARD, asset( ( SCORUM_CURATE_APR * gpo.virtual_supply.amount ) / (
           SCORUM_BLOCKS_PER_YEAR * 100 ), SCORUM_SYMBOL ) );
           auto witness_pay = std::max( SCORUM_MIN_PRODUCER_REWARD, asset( ( SCORUM_PRODUCER_APR *
           gpo.virtual_supply.amount ) / ( SCORUM_BLOCKS_PER_YEAR * 100 ), SCORUM_SYMBOL ) );
           ...
           BOOST_REQUIRE( db.get_account( witness_name ).balance.amount.value == ( old_witness_balance + witness_pay
           ).amount.value );
           ...
           while( db.head_block_num() < SCORUM_START_VESTING_BLOCK - 1)
           {
              ...
              new_rewards = std::max( SCORUM_MIN_CONTENT_REWARD, asset( ( SCORUM_CONTENT_APR * gpo.virtual_supply.amount
           ) / ( SCORUM_BLOCKS_PER_YEAR * 100 ), SCORUM_SYMBOL ) )
                 + std::max( SCORUM_MIN_CURATE_REWARD, asset( ( SCORUM_CURATE_APR * gpo.virtual_supply.amount ) / (
           SCORUM_BLOCKS_PER_YEAR * 100 ), SCORUM_SYMBOL ) );
              witness_pay = std::max( SCORUM_MIN_PRODUCER_REWARD, asset( ( SCORUM_PRODUCER_APR *
           gpo.virtual_supply.amount ) / ( SCORUM_BLOCKS_PER_YEAR * 100 ), SCORUM_SYMBOL ) );
              ...
              BOOST_REQUIRE( db.get_account( witness_name ).vesting_shares.amount.value == ( old_witness_shares +
           witness_pay_shares ).amount.value );
           ...
           BOOST_REQUIRE_EQUAL( gpo.total_reward_fund_scorum.amount.value,
              reward_scorum.amount.value + virtual_supply.amount.value / 10 / SCORUM_BLOCKS_PER_YEAR +
           virtual_supply.amount.value / 10 / SCORUM_BLOCKS_PER_DAY );
           BOOST_REQUIRE_EQUAL( db.get_account( witness_name ).vesting_shares.amount.value,
              old_witness_shares.amount.value + ( asset( ( ( virtual_supply.amount.value / SCORUM_BLOCKS_PER_YEAR ) *
           SCORUM_1_PERCENT ) / SCORUM_100_PERCENT, SCORUM_SYMBOL ) * ( vesting_shares / vesting_scorum ) ).amount.value
           );
           validate_database();
           */
    }
...
BOOST_AUTO_TEST_CASE(liquidity_rewards)
{
    ...
    try
    {
        ...
        BOOST_REQUIRE(
            fill_order_op.open_pays.amount.value == asset(alice_sbd.amount.value / 20, SBD_SYMBOL).amount.value);
        ...
        BOOST_REQUIRE(fill_order_op.open_pays.amount.value
            == asset((alice_sbd.amount.value / 10) * 3 - alice_sbd.amount.value / 20, SCORUM_SYMBOL).amount.value);
        ...
        BOOST_REQUIRE(
            fill_order_op.open_pays.amount.value == asset(alice_sbd.amount.value / 20, SCORUM_SYMBOL).amount.value);
        BOOST_REQUIRE(fill_order_op.current_owner == "alice");
        BOOST_REQUIRE(fill_order_op.current_orderid == 5);
        BOOST_REQUIRE(
            fill_order_op.current_pays.amount.value == asset(alice_sbd.amount.value / 20, SBD_SYMBOL).amount.value);
        ...
        generate_blocks(
            db.head_block_time() + fc::seconds(SCORUM_MIN_LIQUIDITY_REWARD_PERIOD_SEC.to_seconds() / 2), true);
        ...
        BOOST_REQUIRE(
            fill_order_op.current_pays.amount.value == asset(alice_sbd.amount.value / 20, SCORUM_SYMBOL).amount.value);
        ...
        // BOOST_REQUIRE( ops[0].get< liquidity_reward_operation>().payout.amount.value ==
        // SCORUM_MIN_LIQUIDITY_REWARD.amount.value );
        ...
        generate_blocks(sam_reward_last_update + SCORUM_LIQUIDITY_TIMEOUT_SEC - fc::seconds(SCORUM_BLOCK_INTERVAL / 2)
                - SCORUM_MIN_LIQUIDITY_REWARD_PERIOD_SEC,
            true);
...
BOOST_AUTO_TEST_CASE(sbd_stability)
{
    try
    {
        ...
        // Using the debug node plugin to manually set account balances to create required market conditions for this
        // test
        ...
        db_plugin->debug_generate_blocks_until(debug_key,
            fc::time_point_sec(db.get_comment(comment.author, comment.permlink).cashout_time.sec_since_epoch()
                                                   - 2 * SCORUM_BLOCK_INTERVAL),
            true, database::skip_witness_signature);
        ...
        asset sbd_balance
            = asset((gpo.virtual_supply.amount * (SCORUM_SBD_STOP_PERCENT + 30)) / SCORUM_100_PERCENT, SCORUM_SYMBOL)
            * exchange_rate;
        db_plugin->debug_update(
            [=](database& db) {
                db.modify(db.get_account("sam"), [&](account_object& a) { a.sbd_balance = sbd_balance; });
            },
        ...
        auto comment_reward = (gpo.total_reward_fund_scorum.amount + 2000)
            - ((gpo.total_reward_fund_scorum.amount + 2000) * 25 * SCORUM_1_PERCENT) / SCORUM_100_PERCENT;
        comment_reward /= 2;
        auto sbd_reward = (comment_reward * gpo.sbd_print_rate) / SCORUM_100_PERCENT;
        auto alice_sbd = db.get_account("alice").sbd_balance + db.get_account("alice").reward_sbd_balance
            + asset(sbd_reward, SCORUM_SYMBOL) * exchange_rate;
        ...
        db_plugin->debug_update(
            [=](database& db) {
                db.modify(db.get_account("sam"),
                    [&](account_object& a) { a.sbd_balance = asset((194 * sbd_balance.amount) / 500, SBD_SYMBOL); });
            },
            database::skip_witness_signature);
        ...
        while ((db.get_dynamic_global_properties().current_sbd_supply * exchange_rate).amount
            >= (db.get_dynamic_global_properties().virtual_supply.amount * SCORUM_SBD_START_PERCENT)
                / SCORUM_100_PERCENT)
        {
            auto& gpo = db.get_dynamic_global_properties();
