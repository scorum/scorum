...
BOOST_AUTO_TEST_CASE( account_create_apply )
{
   try
   {
      ...
      /// because init_witness has created vesting shares and blocks have been produced, 100 SCORUM is worth less than 100 vesting shares due to rounding
...
BOOST_AUTO_TEST_CASE( comment_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.cashout_time == fc::time_point_sec( db.head_block_time() + fc::seconds( SCORUM_CASHOUT_WINDOW_SECONDS ) ) );
...
BOOST_AUTO_TEST_CASE( vote_apply )
{
   try
   {
...

      {
         ...
         int64_t max_vote_denom = ( db.get_dynamic_global_properties().vote_power_reserve_rate * SCORUM_VOTE_REGENERATION_SECONDS ) / (60*60*24);
         ...
         BOOST_REQUIRE( alice_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / SCORUM_100_PERCENT );
         BOOST_REQUIRE( alice_comment.cashout_time == alice_comment.created + SCORUM_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr->rshares == alice.vesting_shares.amount.value * ( old_voting_power - alice.voting_power ) / SCORUM_100_PERCENT );
         ...
         BOOST_REQUIRE( db.get_account( "alice" ).voting_power == old_voting_power - ( ( old_voting_power + max_vote_denom - 1 ) * SCORUM_100_PERCENT / ( 2 * max_vote_denom * SCORUM_100_PERCENT ) ) );
         BOOST_REQUIRE( bob_comment.net_rshares.value == alice.vesting_shares.amount.value * ( old_voting_power - db.get_account( "alice" ).voting_power ) / SCORUM_100_PERCENT );
         BOOST_REQUIRE( bob_comment.cashout_time == bob_comment.created + SCORUM_CASHOUT_WINDOW_SECONDS );
         BOOST_REQUIRE( itr != vote_idx.end() );
         validate_database();
         ...
         BOOST_REQUIRE( new_bob.voting_power == SCORUM_100_PERCENT - ( ( SCORUM_100_PERCENT + max_vote_denom - 1 ) / max_vote_denom ) );
         BOOST_REQUIRE( new_alice_comment.net_rshares.value == old_abs_rshares + new_bob.vesting_shares.amount.value * ( old_voting_power - new_bob.voting_power ) / SCORUM_100_PERCENT );
         ...
         auto sam_weight /*= ( ( uint128_t( new_sam.vesting_shares.amount.value ) ) / 400 + 1 ).to_uint64();*/
                         = ( ( uint128_t( new_sam.vesting_shares.amount.value ) * ( ( SCORUM_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) ) / SCORUM_100_PERCENT ).to_uint64();

         BOOST_REQUIRE( new_sam.voting_power == SCORUM_100_PERCENT - ( ( SCORUM_100_PERCENT + max_vote_denom - 1 ) / ( 2 * max_vote_denom ) ) );
         ...
         int64_t regenerated_power = (SCORUM_100_PERCENT * ( db.head_block_time() - db.get_account( "alice").last_vote_time ).to_seconds() ) / SCORUM_VOTE_REGENERATION_SECONDS;
         ...
         auto new_rshares = ( ( fc::uint128_t( db.get_account( "alice" ).vesting_shares.amount.value ) * used_power ) / SCORUM_100_PERCENT ).to_uint64();

         BOOST_REQUIRE( db.get_comment( "alice", string( "foo" ) ).cashout_time == db.get_comment( "alice", string( "foo" ) ).created + SCORUM_CASHOUT_WINDOW_SECONDS );
         ...
         old_abs_rshares = new_bob_comment.abs_rshares.value;
         used_power = ( ( SCORUM_1_PERCENT * 25 * ( new_alice.voting_power ) / SCORUM_100_PERCENT ) + max_vote_denom - 1 ) / max_vote_denom;
         ...
         new_rshares = ( ( fc::uint128_t( new_alice.vesting_shares.amount.value ) * used_power ) / SCORUM_100_PERCENT ).to_uint64();
...
BOOST_AUTO_TEST_CASE( withdraw_vesting_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( SCORUM_VESTING_WITHDRAW_INTERVALS * 2 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db.head_block_time() + SCORUM_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
      ...
      BOOST_REQUIRE( alice.vesting_shares.amount.value == old_vesting_shares.amount.value );
      BOOST_REQUIRE( alice.vesting_withdraw_rate.amount.value == ( old_vesting_shares.amount / ( SCORUM_VESTING_WITHDRAW_INTERVALS * 3 ) ).value );
      BOOST_REQUIRE( alice.to_withdraw.value == op.vesting_shares.amount.value );
      BOOST_REQUIRE( alice.next_vesting_withdrawal == db.head_block_time() + SCORUM_VESTING_WITHDRAW_INTERVAL_SECONDS );
      validate_database();
...
BOOST_AUTO_TEST_CASE( feed_publish_apply )
{
   try
   {
      ...
      feed_publish_operation op;
      op.publisher = "alice";
      op.exchange_rate = price( ASSET( "1000.000 TESTS" ), ASSET( "1.000 TBD" ) ); // 1000 SCORUM : 1 SBD
...
BOOST_AUTO_TEST_CASE( convert_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( convert_request->conversion_date == db.head_block_time() + SCORUM_CONVERSION_DELAY );
...
BOOST_AUTO_TEST_CASE( limit_order_create_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( SBD_SYMBOL, SCORUM_SYMBOL ) );
...
BOOST_AUTO_TEST_CASE( account_recovery )
{
   try
   {
      ...
      generate_blocks( db.head_block_time() + ( SCORUM_OWNER_AUTH_RECOVERY_PERIOD - SCORUM_ACCOUNT_RECOVERY_REQUEST_EXPIRATION_PERIOD ) );
      generate_block();
...
BOOST_AUTO_TEST_CASE( change_recovery_account )
{
   try
   {
      ...
      generate_blocks( db.head_block_time() + SCORUM_OWNER_AUTH_RECOVERY_PERIOD - fc::seconds( SCORUM_BLOCK_INTERVAL ), true );
...
BOOST_AUTO_TEST_CASE( escrow_transfer_validate )
{
   try
   {
      ...
      op.sbd_amount.symbol = SCORUM_SYMBOL;
      SCORUM_REQUIRE_THROW( op.validate(), fc::exception );

      BOOST_TEST_MESSAGE( "--- failure when scorum symbol != SCORUM" );
      op.sbd_amount.symbol = SBD_SYMBOL;
      op.scorum_amount.symbol = SBD_SYMBOL;
      SCORUM_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      BOOST_TEST_MESSAGE( "--- failure when sbd == 0 and scorum == 0" );
...
BOOST_AUTO_TEST_CASE( escrow_release_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- failure when scorum == 0 and sbd == 0" );
      op.sbd_amount.amount = 0;
      SCORUM_REQUIRE_THROW( op.validate(), fc::exception );
      ...
      op.sbd_amount.symbol = SBD_SYMBOL;
      op.scorum_amount = ASSET( "1.000 TBD" );
      SCORUM_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "success when amount is SCORUM" );
      op.amount = ASSET( "1.000 TESTS" );
      op.validate();
   }
   FC_LOG_AND_RETHROW()
...
BOOST_AUTO_TEST_CASE( transfer_to_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success transferring SCORUM to self" );
      op.to = "alice";
      ...
      BOOST_TEST_MESSAGE( "--- success transferring SCORUM to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( transfer_from_savings_apply )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing SCORUM to self" );
      op.to = "alice";
      ...
      BOOST_REQUIRE( db.get_savings_withdraw( "alice", op.request_id ).complete == db.head_block_time() + SCORUM_SAVINGS_WITHDRAW_TIME );
      validate_database();
      ...
      BOOST_TEST_MESSAGE( "--- success withdrawing SCORUM to other" );
      op.to = "bob";
      op.amount = ASSET( "1.000 TESTS" );
      op.request_id = 3;
      ...
      generate_blocks( db.head_block_time() + SCORUM_SAVINGS_WITHDRAW_TIME - fc::seconds( SCORUM_BLOCK_INTERVAL ), true );
      ...
      for( int i = 0; i < SCORUM_SAVINGS_WITHDRAW_REQUEST_LIMIT; i++ )
      {
         op.request_id = i;
         tx.clear();
         tx.operations.push_back( op );
      ...
      op.request_id = SCORUM_SAVINGS_WITHDRAW_REQUEST_LIMIT;
      tx.clear();
      tx.operations.push_back( op );
      tx.sign( alice_private_key, db.get_chain_id() );
      SCORUM_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
      BOOST_REQUIRE( db.get_account( "alice" ).savings_withdraw_requests == SCORUM_SAVINGS_WITHDRAW_REQUEST_LIMIT );
      validate_database();
   }
   FC_LOG_AND_RETHROW()
}
...
BOOST_AUTO_TEST_CASE( claim_reward_balance_validate )
{
   try
   {
      ...
      op.reward_scorum = ASSET( "1.000 TESTS" );
...
BOOST_AUTO_TEST_CASE( account_create_with_delegation_apply )
{
   try
   {
      ...
      tx.clear();
      op.fee=asset( db.get_witness_schedule_object().median_props.account_creation_fee.amount * SCORUM_CREATE_ACCOUNT_WITH_SCORUM_MODIFIER * SCORUM_CREATE_ACCOUNT_DELEGATION_RATIO, SCORUM_SYMBOL );
      ...
      fund( "alice" , asset( db.get_witness_schedule_object().median_props.account_creation_fee.amount * SCORUM_CREATE_ACCOUNT_WITH_SCORUM_MODIFIER * SCORUM_CREATE_ACCOUNT_DELEGATION_RATIO , SCORUM_SYMBOL ));
      SCORUM_REQUIRE_THROW( db.push_transaction( tx, 0 ), fc::exception );
...
BOOST_AUTO_TEST_CASE( delegate_vesting_shares_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( alice_comment.net_rshares.value == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / SCORUM_100_PERCENT );
      BOOST_REQUIRE( itr->rshares == bob_acc.effective_vesting_shares().amount.value * ( old_voting_power - bob_acc.voting_power ) / SCORUM_100_PERCENT );
...
BOOST_AUTO_TEST_CASE( comment_beneficiaries_apply )
{
   try
   {
      ...
      BOOST_REQUIRE( db.get_account( "bob" ).reward_vesting_scorum.amount + db.get_account( "sam" ).reward_vesting_scorum.amount == db.get_comment( "alice", string( "test" ) ).beneficiary_payout_value.amount );
      BOOST_REQUIRE( ( db.get_account( "alice" ).reward_sbd_balance.amount + db.get_account( "alice" ).reward_vesting_scorum.amount ) == db.get_account( "bob" ).reward_vesting_scorum.amount + 2 );
      BOOST_REQUIRE( ( db.get_account( "alice" ).reward_sbd_balance.amount + db.get_account( "alice" ).reward_vesting_scorum.amount ) * 2 == db.get_account( "sam" ).reward_vesting_scorum.amount + 3 );
   }
   FC_LOG_AND_RETHROW()
}
