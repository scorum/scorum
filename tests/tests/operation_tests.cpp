...
BOOST_AUTO_TEST_CASE(transfer_from_savings_apply)
{
    try
    {
        ...
        BOOST_REQUIRE(db.get_savings_withdraw("alice", op.request_id).complete
            == db.head_block_time() + SCORUM_SAVINGS_WITHDRAW_TIME);
        validate_database();
...
BOOST_AUTO_TEST_CASE(comment_beneficiaries_apply)
{
    try
    {
        ...
        BOOST_REQUIRE(
            (db.get_account("alice").reward_sbd_balance.amount + db.get_account("alice").reward_vesting_scorum.amount)
            == db.get_account("bob").reward_vesting_scorum.amount + 2);
        BOOST_REQUIRE(
            (db.get_account("alice").reward_sbd_balance.amount + db.get_account("alice").reward_vesting_scorum.amount)
                * 2
            == db.get_account("sam").reward_vesting_scorum.amount + 3);
    }
    FC_LOG_AND_RETHROW()
}
